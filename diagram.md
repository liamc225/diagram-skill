...
- **Type** (optional): `architecture` | `flow` | `sequence` | `erd`
...
2. Open a live preview. First check if Node.js is available by running `node --version`. If it is:
   ...
   const tmpFile = path.join(os.tmpdir(), 'mermaid_url_' + process.pid + '.txt');
   ...
   Then open the URL from the temp file using the platform-appropriate command:
   - macOS: `open "$(cat <tmpFile>)"`
   - Linux: `xdg-open "$(cat <tmpFile>)"`
   - Windows/WSL: `wslview "$(cat <tmpFile>)"` or `cmd.exe /c start "$(cat <tmpFile>)"`

   If Node.js is NOT available, tell the user to copy the Mermaid code block and paste it into https://mermaid.live manually.
